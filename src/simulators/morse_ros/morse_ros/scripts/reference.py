...
vel_msg = Twist()
orca_human2_pub = rospy.Publisher(
    "/morse_agents/human2/cmd_vel", Twist, queue_size=10, latch=False
)
orca_human3_pub = rospy.Publisher(
    "/morse_agents/human3/cmd_vel", Twist, queue_size=10, latch=False
)
...
def get_plan(orca_agent):
    ...
    start.pose.position.x = (
        orca_agent.current_pose.pose.position.x
    )  #  database.start_human2_pose_x
    start.pose.position.y = (
        orca_agent.current_pose.pose.position.y
    ...
    Goal.pose.position.x = (
        orca_agent.goal_pose.pose.position.x
    )  # database.goal_human2_pose_x
    Goal.pose.position.y = (
        orca_agent.goal_pose.pose.position.y
...
def velocity(plan, orca_agent, i, tfBuffer):
    ...
    x_vel_world = (
        plan.path[i].pose.position.x - orca_agent.current_pose.pose.position.x
    ) / (
        0.05
    )  # can be i-1
    y_vel_world = (
        plan.path[i].pose.position.y - orca_agent.current_pose.pose.position.y
    ) / (
        0.05
    )  # (plan.plan.poses[1].pose.position.y - plan.plan.poses[0].pose.position.y) / (0.05)

    orientation_list = [
        plan.path[i].pose.orientation.x,
        plan.path[i].pose.orientation.y,
        plan.path[i].pose.orientation.z,
        plan.path[i].pose.orientation.w,
    ]
...
def add_orca_agents(sim, general_database, orca_agent_1, orca_agent_2):
    ...
    general_database.human2_orca = sim.addAgent(
        (
            orca_agent_1.current_pose.pose.position.x,
            orca_agent_1.current_pose.pose.position.y,
        )
    )
    general_database.human3_orca = sim.addAgent(
        (
            orca_agent_2.current_pose.pose.position.x,
            orca_agent_2.current_pose.pose.position.y,
        )
    )
...
def move_logic(sim, general_database, new_plan, tfBuffer, i, orca_human1, orca_human2):
    ...
    if i == 1:
        ...
        if (
            np.linalg.norm(
                [
                    orca_human1.goal_pose.pose.position.x
                    - orca_human1.current_pose.pose.position.x,
                    orca_human1.goal_pose.pose.position.y
                    - orca_human1.current_pose.pose.position.y,
                ]
            )
            < 0.2
            or len(new_plan.path) < 2
        ):
            ...
            sim.setAgentPrefVelocity(
                general_database.human2_orca, (x_vel, y_vel)
            )  # check this sequence
            sim.doStep()
            sim.setAgentPosition(
                general_database.human3_orca,
                (
                    orca_human2.current_pose.pose.position.x,
                    orca_human2.current_pose.pose.position.y,
                ),
            )
            sim.setAgentPosition(
                general_database.human2_orca,
                (
                    orca_human1.current_pose.pose.position.x,
                    orca_human1.current_pose.pose.position.y,
                ),
            ...
            new_x_vel = orcavel_x * math.cos(yaw) + orcavel_y * math.sin(
                yaw
            )  # vel*math.cos(yaw)
            new_y_vel = -orcavel_x * math.sin(yaw) + orcavel_y * math.cos(
                yaw
            )  # vel*math.sin(yaw)
    ...
    if i == 2:
        ...
        if (
            np.linalg.norm(
                [
                    orca_human2.goal_pose.pose.position.x
                    - orca_human2.current_pose.pose.position.x,
                    orca_human2.goal_pose.pose.position.y
                    - orca_human2.current_pose.pose.position.y,
                ]
            )
            < 0.2
            ...
            sim.setAgentPrefVelocity(
                general_database.human3_orca, (x_vel, y_vel)
            )  # check this sequence
            sim.doStep()
            sim.setAgentPosition(
                general_database.human2_orca,
...
def main():
    ...
    rospy.Subscriber(
        "/morse_agents/human1/odom", Odometry, update_human1_pose, general_database
    )

    # extension tip 3, have one function and string match subscribers
    rospy.Subscriber(
        "/morse_agents/human2/odom",
        Odometry,
        update_orca_human_pose_callback,
        orca_human1,
    )
    rospy.Subscriber(
        "/morse_agents/human3/odom",
        Odometry,
        update_orca_human_pose_callback,
        orca_human2,
    )
    ...
    while not rospy.is_shutdown():  # try normal while loop also for making it faster
        ...
        move_logic(
            sim, general_database, new_plan_hum_1, tfBuffer, 1, orca_human1, orca_human2
        )
        move_logic(
            sim, general_database, new_plan_hum_2, tfBuffer, 2, orca_human1, orca_human2
        )
