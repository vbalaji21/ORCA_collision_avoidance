...
class BroadCastTF(object):
    def __init__(self):
        ...
        rospy.Timer(rospy.Duration(1.0 / 100), self.tfPub)
        r = rospy.Rate(100)
    ...
    def tfPub(self, event):
        if self.a_pose:
            ...
            self.br.sendTransform(
                (self.a_pose.pose.pose.position.x, self.a_pose.pose.pose.position.y, 0),
                (
                    self.a_pose.pose.pose.orientation.x,
                    self.a_pose.pose.pose.orientation.y,
                    self.a_pose.pose.pose.orientation.z,
                    self.a_pose.pose.pose.orientation.w,
                ),
                now,
